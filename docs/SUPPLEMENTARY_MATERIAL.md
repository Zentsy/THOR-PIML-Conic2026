...
$$
\begin{aligned}
\mathbf{h}_{\text{raw}}, (\mathbf{h}_n, \mathbf{c}_n) &= \text{LSTM}_{2\text{-layers}}(\mathbf{X}_{\text{trunk}}, d_{\text{hidden}}=128) \\
\mathbf{h}_{\text{lstm}} &= \text{LayerNorm}\left(\mathbf{W}_{\text{proj}}\mathbf{X}_{\text{trunk}} + \mathbf{h}_{\text{raw}}\right) \in \mathbb{R}^{B \times 30 \times 128}
\end{aligned}
$$
...
$$
\begin{aligned}
\mathbf{g}_t &= \sigma\left(\mathbf{W}_{\text{gate}} [\mathbf{h}_{\text{lstm}}[t] \parallel \mathbf{h}_{\text{tcn}}[t]] + \mathbf{b}_{\text{gate}}\right) \in [0, 1]^{128} \\
